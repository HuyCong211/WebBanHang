...
jQuery(document).ready(function($)
{
	...
	*/
    ...
    function initIsotopeFiltering()
    {
    	...
    	if($('.product-grid').length)
    	{
	        ...
	        {
				// Lấy giá trị min và max từ hidden input
				var priceRange = $('#amount').val();
				var from = $('#FromAmount').val().replace(/[^\d]/g, '');
				var to = $('#ToAmount').val().replace(/[^\d]/g, '');

				// Lấy URL hiện tại (vd: /san-pham/ao-nam)
				var currentUrl = window.location.pathname;
				var queryParams = new URLSearchParams(window.location.search);

				// Gắn thêm tham số vào URL
				queryParams.set('minPrice', from);
				queryParams.set('maxPrice', to);
	...
	*/
	function initPriceSlider() {
		...
		// Giá trị mặc định
		const minValue = minFromUrl ? parseInt(minFromUrl) : 100000;
		const maxValue = maxFromUrl ? parseInt(maxFromUrl) : 2000000;
		...
		$("#slider-range").slider({
			...
			slide: function (event, ui) {
				$("#amount").val(
					ui.values[0].toLocaleString("vi-VN") + "đ - " +
					ui.values[1].toLocaleString("vi-VN") + "đ"
				);
		...
		$(".filter_button").click(function () {
			const min = $("#FromAmount").val();
			const max = $("#ToAmount").val();
